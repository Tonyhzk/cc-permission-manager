...
### 新增
- **Hook 配置健康检测** - 新增 `checkHookStatus()` 函数，检测四种状态：已安装、部分缺失、已被修改、未安装。区分配置缺失（partial）和用户主动修改命令（modified）
- **自动修复** - 检测到 partial 状态时自动修复 Hook 配置。通过开关控制，localStorage 持久化，默认开启
- **手动修复按钮** - modified 状态（用户主动修改了命令）时仅显示警告和手动修复按钮，不自动覆盖
- **状态徽章** - Hook 状态显示不同颜色徽章：绿色（已安装）、黄色（配置异常）、蓝色（命令被修改）
- **绕过权限模式** - 新增第四种权限模式 `bypassPermissions`，与计划模式、默认模式、自动接受模式并列。此模式默认开启所有权限（最高权限级别），对应 Claude Code 的 `--bypass-permissions` 标志

### 变更
- 版本号从 0.4.4 升至 0.5.0
- Tauri 依赖版本钉死为精确版本，避免跨平台构建时版本漂移
