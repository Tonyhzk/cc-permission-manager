...
- **全局权限配置** - 管理不同模式下的权限设置
  ...
  - 绕过权限模式 (Bypass Permissions Mode)
- **分类管理** - 配置工具和命令的分类权限
- **通知设置** - 自定义通知行为和声音
- **项目设置** - 按项目配置权限
- **多语言支持** - 支持英文和中文界面
...
- 实时预览配置变更
- 一键安装/卸载全局 Hook
- Hook 健康检测：四种状态（已安装、破损、脚本被修改、未安装）
- 自动修复：检测到 Hook 配置破损或脚本过期时自动修复（可开关）
- 关闭自动修复时显示手动修复按钮和提示
- 状态徽章：不同健康状态用不同颜色标识
- 支持深色/浅色主题
- 跨平台支持 (Windows / macOS)
...
在「全局配置」标签页中：
- 选择要配置的模式（计划模式/默认模式/接受编辑模式/绕过权限模式）
- 为每个工具分类设置权限级别
- 点击「保存」应用更改
...
- **`.claude/` 目录保护**：Claude Code 对 `.claude/` 目录（包括其子目录 `rules/`、`hooks/` 等）有内置的安全保护。即使 Hook 返回 `allow`，Claude Code 仍会针对该目录下的文件操作弹出权限确认窗口。这是 Claude Code 自身的安全机制，不受 Hook 脚本控制。对于工作目录内外的其他路径，Hook 的权限决策可正常生效。
